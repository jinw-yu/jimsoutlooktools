...
[![版本](https://img.shields.io/badge/版本-v1.1.1-blue)](https://gitee.com/jasonw-yu/jtools-outlook)
...
- 📈 **差异分析**：自动分析并显示每个年份的邮件数量
- ✅ **选择性同步**：支持选择特定年份进行下载
- 📊 **进度显示**：实时显示同步进度，支持中途取消
- 🛡️ **错误隔离**：个别邮件同步失败不影响整体流程

### 导入邮件
- 📥 **批量导入**：从本地 .msg 文件批量导入到 PST 文件夹
- 🔍 **智能去重**：使用 Message-ID 检测重复邮件，自动跳过
- ⚡ **性能优化**：HashSet 预加载已存在邮件，快速去重
- 📁 **目标选择**：支持选择目标 PST 和文件夹
